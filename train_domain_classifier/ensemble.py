...
def ensembled_prediction(data_type):
    dark_path = '../eval/results/%s-dark-epoch100-open-pose-single-scale-200/' \
                '%s-dark-epoch100-200_result.json' \
                % (data_type, data_type)
    mb_path = '../eval/results/%s-motion_blur-epoch100-open-pose-single-scale-200/' \
              '%s-motion_blur-epoch100-200_result.json' \
              % (data_type, data_type)
    ours_path = '../eval/results/%s-ours-epoch100-open-pose-single-scale-200/' \
                '%s-ours-epoch100-200_result.json' \
                % (data_type, data_type)
    ...
    output_folder = '../eval/results/%s-ensemble-epoch100-open-pose-single-scale-200' % data_type
    ours_folder = '../eval/results/%s-ours-epoch100-open-pose-single-scale-200/' % data_type
    mb_folder = '../eval/results/%s-motion_blur-epoch100-open-pose-single-scale-200/' % data_type
    dark_folder = '../eval/results/%s-dark-epoch100-open-pose-single-scale-200/' % data_type
    ...
    for dic in cls_json:
        ...
        if 'frames' == data_type:
            file_name = 'result_' + img_id + '.png'
        elif 'frames_dark' == data_type:
            file_name = 'result_' + img_id + '.jpg'
        elif 'frames_motion_blur' == data_type:
            file_name = 'result_' + img_id + '.jpg'
        else:
        ...
        if classifications == 0:
            ...
            if os.path.exists(ours_folder + 'predictions/' + file_name):
            ...
            if os.path.exists(dark_folder + 'predictions/' + file_name):
            ...
            if os.path.exists(mb_folder + 'predictions/' + file_name):
                shutil.copyfile(mb_folder + 'predictions/' + file_name, prediction_folder + file_name)

        # copy prediction file from
        ensembled_res += res

    ensembled_res_path = output_folder + '/%s-ensemble-epoch100-200_result.json' % data_type
...
if __name__ == '__main__':
    ...
    parser.add_argument('--coco_dataType', type=str, default='frames_motion_blur',
                        help='val2014_random1k or val2014_random1k_resolution or val2014_random1k_motion_blur'
                             'or val2014_random1k_dark or '
                             'frames')

...
parser.add_argument('--gpu', type=str, default='3', help='0-7')
...
LOGS_DIR = "./logs"
WEIGHT_DIR = "./weights" + args.effect
...
def get_last_epoch_and_weights_file():
    ...
    if len(epochs) == 0:
        ...
        print('successfully identify maximum ep')
...
# prepare generators
train_client = DataIterator("../dataset/train_dataset_2014%s.h5" % args.effect, shuffle=True, augment=True,
                            batch_size=batch_size)
val_client = DataIterator("../dataset/val_dataset_2014%s.h5" % args.effect, shuffle=False, augment=False,
                          batch_size=batch_size)
...
multisgd = MultiSGD(lr=base_lr, momentum=momentum, decay=0.0, nesterov=False, lr_mult=lr_mult, name='MultiSGD')
...
model.compile(loss=eucl_loss, optimizer=tf.keras.optimizers.Adam(1e-3))
